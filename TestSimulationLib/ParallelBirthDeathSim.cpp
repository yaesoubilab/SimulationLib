#include "ParallelBirthDeathSim.h"

namespace SimulationLib {
    ParallelBirthDeathSim::ParallelBirthDeathSim(string _fileName,        \
      int _nTrajectories, int _timeMax, int _periodLength, long _nPeople, \
    ...
    ParallelBirthDeathSim::~ParallelBirthDeathSim(void) {
    ...
    void ParallelBirthDeathSim::Run(void) {
        ...
        for (int i = 0; i < nTrajectories; ++i)
        {
            ...
            futures[i] = async(launch::async | launch::deferred, \
                               &ParallelBirthDeathSim::_runTrajectory, \
                               this, \
                               birthsArr[i], \
                               deathsArr[i], \
                               populationArr[i], \
        ...
        // Wait for all threads to finish running '_runTrajectory'. Acts as a
        // barrier.
        for (int i = 0; i < nTrajectories; ++i)
        {
            futures[i].wait();
    ...
    void ParallelBirthDeathSim::_runTrajectory(IncidenceTimeSeries<int>  *births,     \
    ...
    void ParallelBirthDeathSim::reportStats(int t, long nPeople, int nBirths, int nDeaths) {
    ...
    //   the population.
    void ParallelBirthDeathSim::refreshDists(int nPeople,              \
                                             Binomial **bDistribution, \
                                             Binomial **dDistribution) {
